...
### Shipped in v1.7.2
- **Rounds** — a new page (`next_steps.py`, `/api/next_steps`) that ranks the five workflows into fixed Baseline / Ongoing / On demand sections, each row in a computed state, plus a monotonic prize layer of ~730 named tiers across 40 ladders and 104 feats. Never bottoms out (including for a library already in good shape — see the aspirational-tier ladders), never nags off-page. Design in `prompts/NEXT_STEPS.md`

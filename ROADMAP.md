...
### Shipped in v1.7.2
- **Rounds** — a new page (`rounds.py`, `/api/next_steps`) that ranks the five workflows into fixed Baseline / Ongoing / On demand sections, each row in a computed state, plus a monotonic prize layer of individually named tiers and one-off feats. Never bottoms out (including for a library already in good shape — see the aspirational-tier ladders), never nags off-page. Design in `prompts/NEXT_STEPS.md`
...
### Shipped in v1.7.3
- **"How you got here"** — a dated achievement record (`ns_progress['history']`, `_timeline`), written in the same pass that advances the latches so the two cannot drift. An install upgrading into it gets `history_from_runs` — everything the audit log can prove, dated retroactively per distinct audit day — and one `prior` marker for the genuinely undatable remainder. Each row carries the ladder's `terms` phrase, so a rung says what it was earned *on*, not just where it sits
- **An aspirational tier for the prize shelf** — the layer went dead for exactly the user who had done the work: cleanliness feats are all first-scan zero states, and the zombie ladders only pay when a mess returns. Eleven new ladders (Conservator, Unblemished, Full Marks, Fire Brigade, Atlas, Old Faithful, Librarian, Videophile, Provenance, Singleton, Matchmaker) and new feats crossing the zero states with scale and time
- **Every workflow's prize is now work done, not bytes held** — `LADDER_OWNER` no longer points a workflow row at a library-state ladder; `test_no_workflow_card_is_paid_in_bytes` fails the build if one comes back. Packrat / Tidiness / Purity were also one number on three tiles, and are rebased
- **Workflow pages no longer go stale after you act on a row** — an `auditorr:audit_complete` event refreshes each page's own loader, a module-level `DISMISSED` set survives remounts, and the actions the filesystem watcher cannot see (keep-files removal, added exclusions) call `nudge_watchdog()`. Refresh buttons removed
- **Multi-instance Sonarr/Radarr fixes (#22)** — `normalize_arr_connections` merges the singleton keys with `ARR_CONNECTIONS` rather than picking one; Backfill's Sonarr grouping key includes the connection id; per-series failures no longer discard a whole library; bulk listings get `_ARR_LIST_TIMEOUT`; unreadable instances are reported rather than silently absent
- **File Explorer's status filter split (#23)** — `status` is only ever a status; Duplicates and Excluded became `+`/`-` `FlagToggle`s, so "orphaned but not excluded" is expressible. `EXCLUSION_HIDE_FROM_EXPLORER` becomes the toggle's starting position instead of a hard filter
- **Backfill credit at grab time** — `record_backfill` fires when the arr accepts the release, not from the in-memory import watch, which could not survive a container restart mid-download. Both event counters now write through `db_update_meta`, and the audit merges rather than overwrites them

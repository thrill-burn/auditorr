...
def generate_script(script_type, results, cfg):
    ...
    if script_type == 'orphaned_torrents_delete':
        ...
        lines += [
            ...
            '#   3. Delete each orphaned file with progress output',
        ...
        for i, f in enumerate(orphaned):
            ...
            size_bytes = f['size']
            lines += [
                f'# File {i+1}/{len(orphaned)}: {filename} — {_human_size(size_bytes)}',
                f'printf "[{i+1}/{len(orphaned)}] Deleting: %s ({_human_size(size_bytes)})\\n" {qname}',
                f'if [ -f {qfull} ]; then',
